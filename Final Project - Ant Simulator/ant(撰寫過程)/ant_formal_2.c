...
void Show_Cursor(int visible) { //顯示隱藏光標
...
/*設置蟻巢(3x3)*/
    srand(time(NULL));
    home_row = rand() % (row_side - 4) + 4;    //side-3以避免蟻巢超出地圖邊界
    home_line = rand() % (line_side - 4) + 4;
    ...
    for (int count = 0; count < ant_movetimes; count++) {
        ...
        case 2:;    //go back to home
            ...
            */
            ...
            map[now][current->row][current->line] = ' ';
            hormone[current->row][current->line] = 50;
            ...
            if (abs(row - home_row) <= 2) {	//判斷是否抵達家中
                if (abs(line - home_line) <= 2) {    //抵達家的四週
                    current->job = 1;
                    current->walk = (current->walk)*1.25 + (walk_times*(1/5));
                    do {                            //重新設定目的地
                        current->desrow = rand() % (row_side)+1;
                        current->desline = rand() % (line_side)+1;
                    } while (map[now][current->desrow][current->desline] == '#');
...
void farthest(struct ant *current){		//go to farthest
    ...
    map[now][row][line] = ' ';
    if(map[next][len[0][1]][len[0][2]]=='-'||map[next][len[0][1]][len[0][2]]=='|'){
        map[next][row][line]='@';
    }
    else{
