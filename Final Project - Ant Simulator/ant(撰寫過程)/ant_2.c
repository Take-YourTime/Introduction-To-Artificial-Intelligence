...
void generate_new_ant(int len,int now){
    ...
    while(true){
	    new->row = rand()%(row_side)+1;
	    new->line = rand()%(line_side)+1;
//隨機生成於蟻巢周圍，不是隨機生成於地圖上       
...
/*now -> next*/
        ...
        else if(line>home_line+2){//正上方 (執行不到)
        ...
        if( (ptr->row <= home_row+2) && (ptr->row >= home_row) && (ptr->line <= home_line+2) && (ptr->line >= home_line) ){
            ...
            while(true){
                ptr->row = rand()%row_side+1;
                ptr->line = rand()%line_side+1;
                if(map[now][ptr->row][ptr->line]!='@'){//要生成螞蟻的位置沒有螞蟻才可以生成
                    map[now][ptr->row][ptr->line]='@';
                    break;
                }
            }
/*螞蟻每走一步，步數-1。每次回家增加可走步數*/
            ptr->walk = 0;
            return 1;//表示需要生成新的螞蟻
        }
