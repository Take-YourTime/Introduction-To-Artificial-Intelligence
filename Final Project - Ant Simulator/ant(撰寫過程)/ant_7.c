...
void ant_detect(struct ant* a, int);
...
int main() {
    ...
    int ant_movetimes, food_count, food_times;
    int ant_len = 0;    //有幾隻螞蟻
    int tick = 1;        //第幾代
    ...
    /*-------------*/
        //設置初始螞蟻
    for (int i = 0; i < 2; i++) {
        generate_new_ant(rand() % row_side + 1, rand() % line_side + 1);
        ant_len++;
    }
    ...
    while (true) {
        ...
        while (current != NULL) {    //走訪每隻螞蟻
            (current->walk)--;
            printf("[%d %d] ", current->row, current->line);
        ...
        for (int i = 0; i < row_side + 2; i++) {    //將食物傳到地圖上
            for (int j = 0; j < line_side + 2; j++)
                if (food[i][j] > 0)
...
void ant_detect(struct ant* current, int ant_movetimes) {    //偵測螞蟻要做甚麼工作
    ...
    for (int count = 0; count < ant_movetimes; count++) {
        ...
        case 1:;    //find food
            ...
            printf("【%d %d】", current->desrow, current->desline);
            ...
            */
            ...
            for (i = 0; i < 3; i++) {        //至多執行三次
                ...
                if (map[next][way[det].row][way[det].line] != '@') {    //判斷該位置有沒有同伴
                    ...
                    way[det].distance = 0.0;    //有同伴時，該位置不作為選項，並繼續找到下一個近的位置
                    det = 0;
                    min_distance = 0;
                }
            }

            if (i == 2) {    //若三個位置都有同伴，則留在原地
                map[now][row][line] = ' ';
                map[next][row][line] = '@';
            }
...
void ant_normalmove(int desrow, int desline, int* row, int* line) {
    ...
    for (int count = 0; count < 9; count++) {    //找最小距離
        double min_lenth = DBL_MAX;
